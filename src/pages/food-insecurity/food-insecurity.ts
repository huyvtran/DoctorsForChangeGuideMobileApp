...
export class FoodInsecurity {
  ...
  isFavorite;
  toggleFavorite(){
    this.storageService.toggleFavoritePage(FoodInsecurity).then(isFavorite => this.isFavorite = isFavorite);
  }

  ionViewDidLoad() {
    this.storageService.isFavorite(FoodInsecurity).then(isFav => this.isFavorite = isFav);
    this.loadContent();
  }
